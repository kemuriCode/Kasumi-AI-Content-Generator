...
final class SettingsController {
	...
	public function import_settings( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		if ( empty( $json ) ) {
			return new WP_Error(
				'missing_json',
				__( 'Brak danych JSON do zaimportowania.', 'kasumi-ai-studio' ),
				array( 'status' => 400 )
			);
		}
	...
	public function reset_settings( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		if ( ! $deleted ) {
			return new WP_Error(
				'reset_failed',
				__( 'Nie udało się zresetować ustawień.', 'kasumi-ai-studio' ),
				array( 'status' => 500 )
			);
		}
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Ustawienia zostały zresetowane do domyślnych.', 'kasumi-ai-studio' ),
			),
			200
		);
	}

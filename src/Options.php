...
final class Options {
	...
	public const OPTION_GROUP = 'kasumi-ai-studio-settings';
	...
	public static function import( string $json ): array {
		...
		if ( json_last_error() !== JSON_ERROR_NONE ) {
			return array(
				'success' => false,
				'message' => __( 'Nieprawidłowy format JSON.', 'kasumi-ai-studio' ),
			);
		}
		...
		if ( ! is_array( $data ) ) {
			return array(
				'success' => false,
				'message' => __( 'Dane muszą być tablicą.', 'kasumi-ai-studio' ),
			);
		}
		...
		return array(
			'success' => true,
			'message' => __( 'Ustawienia zostały zaimportowane.', 'kasumi-ai-studio' ),
			'data'    => $sanitized,
		);
	}
